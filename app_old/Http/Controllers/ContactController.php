...
class ContactController extends Controller
{
    ...
    public function serverSide(Request $request,$org_id,$type='')
    {
      ...
      foreach($contacts as $key=>$contact)
      {
        ...
        $data[$key][]=$tag_html;
        $data[$key][]='<a class="attachment-view-btn d-flex align-items-center" data-files='.json_encode($contact->attachment).' data-toggle="modal" data-target="#attachment-view" data-file-type="contact" data-id="'.$contact->id.'"><img src="/images/site-images/doc-attch.svg">'.count($contact->attachment).'
                </a>';
  ...
  public function contactToMerge(Request $request,$org_id){
    ...
    public function view($org_id,$contact_id)
    {
      ...
      return view('contact/view',compact('contact_detail','org_id','groups','contactGroup','contacts'));
    }
